...
namespace cdes_presets
{
    ...
    public class Sliden : UdonSharpBehaviour
    {
        ...
        {
            var duration = _videoPlayer.GetDuration(); //视频时常
            var pageCount = (uint)Mathf.Floor(duration); //取整（砍掉小数点后面），将浮点值转换为无符号整数，

            MaxPage = pageCount - 1; //最大页码
            Page = 0; //当前页数 = 0
            _step = duration / pageCount; //计算每页代表的视频时长（单位：秒）
            _overrun = Mathf.Abs(duration - pageCount); //总时长与整数页数的误差值
            _guardLoadTime = Time.realtimeSinceStartup + 5; //超时保护
